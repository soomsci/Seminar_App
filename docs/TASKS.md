...
3. 메인 화면: 현재 단계 표시(`onMeta`), 🟢/🔴 큰 버튼(`setStatus`), 빨강 시 코멘트 입력(200자)
4. 리셋 대응: 다음 단계 시 "N단계 진행 중" 알림, 다시 확인(round 증가) 시 재확인 안내 + 진동 (SPEC §3.1)
...
4. 조작 버튼: "다음 단계 →"(`nextStep`) + "🔄 다시 확인"(`restartStep`) — 오조작 방지 confirm 포함 (SPEC §3.2)
...
1. 통합 테스트: 실제 Firebase로 강사 1 + 연수생 2 창을 띄워 전 시나리오 검증
   ...
   - 다중 브라우저 확인: 이 기기에서 가능한 범위(Chrome + Safari 최소)로 연수생 화면 동작 확인. Chromium 전용 API 사용 여부 코드 검사 포함 (SPEC §2: 웨일·삼성 인터넷·인앱 브라우저까지 지원)
...
세션별 상세 시작 프롬프트는 **`docs/PROMPTS.md`** 에 있다. 세션을 열고 권장 모델 설정 후 통째로 붙여넣으면 된다.
