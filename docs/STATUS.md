...
- 요청:
  ...
  - **세션 C**: [QA-3], [QA-4]는 **총괄매니저 승인되어 SPEC §3.2에 요구사항으로 반영됐습니다**(커밋 `134b991`). 이제 개선 제안이 아니라 구현 대상입니다.
...
#### [QA-3] (세션 C / **승인됨 — SPEC §3.2 반영**) 참가자 50명이면 프로젝터에서 조작 버튼이 화면 밖으로 밀린다

- **상태**: 총괄매니저 승인, SPEC §3.2에 요구사항 추가됨(`134b991`). 세션 C 구현 대상.
- **심각도**: 하 (리포트 시점엔 명세 위반 아니었음, 운영 불편)
- **재현**: 1280×800 뷰포트에서 참가자 50명(SPEC §7 설계 상한)일 때 격자가 12열×5행이 되고, "다음 단계 →" 버튼의 top이 864px가 되어 화면(800px) 밖으로 나간다. 26명일 때는 top 668px로 문제없다.
- **영향**: 강사가 단계를 넘기려면 스크롤해야 하고, 스크롤하면 진행률 %가 화면에서 사라진다.
- **제안**: 조작 버튼 영역을 `position: sticky; bottom: 0`으로 고정하거나, 인원이 많을 때 원 크기를 줄이는 것.
- **재검증 방법**(세션 C 수정 후): 1280×800에서 참가자 50명일 때 "다음 단계 →" 버튼의 `getBoundingClientRect().top < innerHeight` 확인.

#### [QA-4] (세션 C / **승인됨 — SPEC §3.2 반영**) 강사가 새로고침하면 진행 중이던 세션으로 돌아갈 수 없다

- **상태**: 총괄매니저 승인, SPEC §3.2에 요구사항 추가됨(`134b991`). 세션 C 구현 대상.
- **심각도**: 하 (리포트 시점 SPEC §3.2에 재입장 요구가 없어 명세 위반은 아니었음)
- **재현**: 강사 대시보드에서 세션 생성 후 새로고침 → "새 세션 시작" 화면으로 돌아가며, 기존 코드로 다시 들어갈 방법이 없다. 새 세션을 만들면 이미 입장한 연수생들과 코드가 어긋난다.
- **영향**: 프로젝터 노트북이 절전에 들어가거나 탭이 새로고침되면 진행 중인 연수를 이어갈 수 없다.
- **제안**: 연수생과 동일하게 localStorage에 세션코드를 저장해 복귀시키거나, 코드 입력으로 기존 세션에 접속하는 경로 추가. v1.1 후보로 검토 요청.
