...
- 요청:
  ...
  - **세션 C**: [QA-3], [QA-4]는 버그가 아니라 명세 공백/개선 제안입니다. 총괄매니저 판단 필요.
...
#### [QA-3] (세션 C / 개선 제안) 참가자 50명이면 프로젝터에서 조작 버튼이 화면 밖으로 밀린다

- **심각도**: 하 (명세 위반 아님, 운영 불편)
...
- **제안**: 조작 버튼 영역을 `position: sticky; bottom: 0`으로 고정하거나, 인원이 많을 때 원 크기를 줄이는 것. (SPEC §3.2를 벗어나지 않는 CSS 범위 조정)

#### [QA-4] (세션 C / 개선 제안) 강사가 새로고침하면 진행 중이던 세션으로 돌아갈 수 없다

- **심각도**: 하 (SPEC §3.2에 재입장 요구가 없으므로 명세 위반은 아님)
